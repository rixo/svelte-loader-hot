...
export default createProxy =>
	function applyHMR(hotOptions, id, moduleHot, component) {
		...
		// (re)render
		if (record) {
			record.reload({ component, hotOptions });
		} else {
		...
		if (!proxy) {
			...
			// tell webpack our HMR is dead, so next update _will_ trigger a full reload
			moduleHot.decline();
		...
		const onError = err => {
			console.log(moduleHot, 'accept error', err);
			// debugger;
			// moduleHot.accept(onError);
		};
		moduleHot.accept(onError);
