...
class ProxyComponent {
	constructor(
		...
		const refreshComponent = (target, anchor, conservativeDestroyCallback) => {
			if (lastError) {
				clearError();
			} else if (conservativeDestroyCallback) {
				const prevCmp = cmp;
				restore = captureState(cmp);
				try {
					createComponent(target, anchor);
					prevCmp.$destroy();
					conservativeDestroyCallback(cmp);
				} catch (err) {
					logError(
						`Failed to recreate ${debugName} instance: ${err && err.stack || err}`
					);
					cmp = prevCmp;
				}
			} else {
...

		{
			...
			try {
				...
				if (current.hotOptions.optimistic) {
					logError(`Failed to create ${debugName} instance: ${err && err.stack || err}`);
					setError(err, target, anchor);
				} else {
					throw err;
				}
